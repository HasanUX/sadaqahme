...
function UmmahProjectDetails() {
  const [details, setDetails] = useState([]);
  const { slug } = useParams();
  //  console.log("use param slug", slug);

  const { ummahProjects } = useContext(StateContext);

  useEffect(() => {
    const singleProject = ummahProjects.map((project) => {
    ...
    <div className="details">
      ...
      <div className="bg-light rounded shadow p-4">
        <h5 className="mb-4 text-dark">
          ...
          যাকাত পাঠানোর সময় রেফারেন্সের সাথে{" "}
        ...
        <h5 className="text-dark">
          ...
          additional referenece when sending Zakat.
        </h5>
      </div>
    </div>
  );
...
export default UmmahProjectDetails;
