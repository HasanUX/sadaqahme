...
function ListFloodDetails() {
  const [details, setDetails] = useState([]);
  const { slug } = useParams();
  const hashTags = ["flood", "donate", "charity", "sadaqah", "fundraise"];
  //  console.log("use param slug", slug);

  const { floodProjects } = useContext(StateContext);

  useEffect(() => {
    const singleProject = floodProjects.map((project) => {
  ...
  return (
    <div className="details">
      <Helmet>
        ...
        <meta
          name="description"
          content="Flood based projects of Sadaqah Made Easy - SME"
        />
      </Helmet>
      <img
      ...
      />
      <h1 className="details__heading">{details.projectHeading}</h1>
      ...
      <p className="details__social">
        <a
          ...
          rel="noreferrer"
        >
          <i className="fa fa-users" aria-hidden="true"></i> find them in web
        </a>
      </p>
      <div className="details__tag_socialFlex">
        <div className="details__socialResponsive">
          ...
          <div className="details__socialShareBtn">
            ...
            <TwitterShareButton url={details.getURL} hashtags={hashTags}>
              <a type="button">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="30"
...
export default ListFloodDetails;
