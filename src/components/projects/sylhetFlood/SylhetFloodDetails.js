...
function SylhetFloodDetails() {
  const [details, setDetails] = useState([]);
  const { slug } = useParams();
  //  console.log("use param slug", slug);

  const { sylhetProjects } = useContext(StateContext);

  useEffect(() => {
    const singleProject = sylhetProjects.map((project) => {
...
export default SylhetFloodDetails;
