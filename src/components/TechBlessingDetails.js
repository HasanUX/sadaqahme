...
function TechBlessingDetails() {
  const [details, setDetails] = useState([]);
  const { slug } = useParams();
  //  console.log("use param slug", slug);

  const { techBlessingProjects } = useContext(StateContext);

  useEffect(() => {
    const singleProject = techBlessingProjects.map((project) => {
  ...
  return (
    <div className="details">
      ...
      {details.projectPlayStore && (
        <>
          <p className="mt-5 mb-4">
            <a
              href={details.projectPlayStore}
              target="_blank"
              className="details__facebookID text-white bg-primary p-2 rounded"
              rel="noreferrer"
            >
          ...
          <p className="mb-5">
            <a
              href={details.projectAppStore}
              target="_blank"
              className="details__facebookID text-white bg-primary p-2 rounded"
              rel="noreferrer"
            >
              Install now on iPhone
            </a>
          </p>
        </>
      )}
      ...
      <b className="details__contact">Contact: {details.projectContact}</b>
      <p className="mt-4">
        <a
          href={details.projectWeb}
          target="_blank"
      ...
      <h5>
        ...
        Jazakallahu khair for adding "<u className="text-success">SME</u>" as
        additional referenece when sending sadaqah / money.
      </h5>
...
export default TechBlessingDetails;
